...
import { Flag, Grid, List } from 'lucide-react';
...
const DefenceApps = () => {
  ...
  const filteredApps = defenceApps.filter(app => {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--color-bg-primary)' }}>
      <PageHeader
        ...
        highlight="Defence"
        subtitle="AI capabilities supporting defence intelligence, procurement, cyber resilience, and geospatial analysis — security-cleared and aligned with MOD AI adoption policy."
        pills={[
          { label: `${defenceApps.length} Apps`, tone: 'accent' },
          { label: 'Security Cleared' },
          { label: 'MOD AI Policy Compliant' },
        ]}
      />
      ...
      <div style={{ padding: '32px var(--section-padding-x)' }}>
        <div style={{ marginBottom: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12, flexWrap: 'wrap' }}>
            <SearchBar
              placeholder="Search Defence apps..."
              value={searchTerm}
          ...
          <p style={{ fontSize: 13, color: 'var(--color-text-muted)' }}>
            Showing {filteredApps.length} of {defenceApps.length} apps
          </p>
        </div>
