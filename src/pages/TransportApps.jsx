...
import { Train, Grid, List } from 'lucide-react';
...
const TransportApps = () => {
  ...
  const filteredApps = transportApps.filter(app => {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--color-bg-primary)' }}>
      <PageHeader
        ...
        highlight="Transport"
        subtitle="AI solutions for traffic management, rail optimisation, road safety, and incident response — aligned with DfT digital standards and UK transport data governance frameworks."
        pills={[
          { label: `${transportApps.length} Apps`, tone: 'accent' },
          { label: 'GDS Compliant' },
          { label: 'Open Transport Data' },
        ]}
      />
      ...
      <div style={{ padding: '32px var(--section-padding-x)' }}>
        <div style={{ marginBottom: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12, flexWrap: 'wrap' }}>
            <SearchBar
              placeholder="Search Transport apps..."
              value={searchTerm}
          ...
          <p style={{ fontSize: 13, color: 'var(--color-text-muted)' }}>
            Showing {filteredApps.length} of {transportApps.length} apps
          </p>
        </div>
